 ...
 * @date    2015-04-14 13:40:37
 * @version $Id$
 */

//淡入，淡出效果
var addFadeEvent = function(obj,param){
    var ul = null, isAllow = true;
    var liList = [];     
    var curindex = 0, lilength = 0;
    ...
    var init = function(){
        ...
        ul = $(obj).children("ul");
        liList = $(ul).children("li").css({position: 'absolute', left:0, top: 0, opacity: 0});
        lilength = liList.length;
        param = param || {};
        $.extend(option, param);

        $(liList[curindex]).css({opacity: 1});
        initScrollListener();
    }
    ...
    var scrollImg = function(dir){
        if(!isAllow)return;
        isAllow = false;
        setTimeout(function(){isAllow = true;}, option.dur / 2);
        var curobj, tarobj;
        ...
        $(tarobj).css({zIndex: -1, opacity: 1, left: -20}).css("filter",'alpha(opacity=100)').stop().animate({left: 0}, option.dur);
        $(curobj).css({zIndex: 0}).stop().animate({left: 0, opacity: 0},option.dur).css("filter","alpha(opacity=100)");
